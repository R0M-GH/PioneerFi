<template>
	<div class="location" :expanded="expandState">
		...
		<span class="status-value" :status="data.curr_status.status">{{
			statusStrings[data.curr_status.status]
		}}</span>
		</div>
		<div class="speed">
		<span class="location-info-type">Speed:</span>
		<span>{{
			data.curr_status.status === "up"
			? `${data.curr_status.down}↓ ${data.curr_status.up}↑`
			: "--"
		}}</span>
		</div>
		<div class="ping">
		...
		<span>{{
			data.curr_status.status === "up" ? `${data.curr_status.ping}ms` : "--"
		}}</span>
		</div>
		<div class="toggle">
		<button @click="expand()">
...
onMounted(() => {
	...
	Highcharts.chart(
		"down-graph",
		buildChartOptions("Download (mbps)", props.data.obs.down)
	);
	Highcharts.chart(
		"up-graph",
		buildChartOptions("Upload (mbps)", props.data.obs.up)
	);
	Highcharts.chart(
		"ping-graph",
		buildChartOptions("Ping (ms)", props.data.obs.ping)
	);
});
