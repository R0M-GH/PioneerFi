...
def get_observations():
    ...
    for node in found_nodes:
        ...
        node["obs"] = node_obs
        curr_status = 'down' if int(time.time()) - \
            120 > node_obs[-1]["timestamp"] else 'up'
        status_obj = {
            'status': curr_status
        }
        if (curr_status == 'up'):
...
def add_observation():
    ...
    print(obs_data)
    # todo: validate obs
    # find the right node by hostname
    matched_node = nodes.find_one({"hostname": obs_data["hostname"]})
    print(matched_node)
